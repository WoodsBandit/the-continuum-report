...
| Attribute | Value |
|-----------|-------|
| **Type** | [Relationship Type - e.g., Professional, Social, Legal] |
| **Direction** | [Bidirectional/One-directional] |
...
For each connection documented:
...
## CONNECTION DOCUMENTATION FLOW

When documenting a connection between Entity A and Entity B:

```
1. FIND THE QUOTE
   └─ Search source documents for mentions of both entities
   └─ Extract exact quote showing the connection

2. CITE THE SOURCE
   └─ Document ID (e.g., ECF 1328-44)
   └─ Page number
   └─ Link to hosted PDF

3. WRITE THE SUMMARY
   └─ One sentence explaining what the connection is
   └─ Be factual, not interpretive

4. DETERMINE RELATIONSHIP TYPE
   └─ Professional, Social, Legal, Financial, etc.
   └─ Can be multiple types
```

**Example Applications:**

| Connection | Relationship Type | Quote + Source |
|------------|-------------------|----------------|
| Epstein → Maxwell | Professional + Social | "Ghislaine was present when I first arrived" (ECF 1328-44, p. 89) |
| Clinton → Epstein | Social | "I flew on Jeffrey's plane with Bill Clinton" (ECF 1328-44, p. 67) |
| Dershowitz → Epstein | Legal + Social | "Dershowitz represented Epstein in..." (ECF 1331-12, p. 23) |
...
# Note: type, evidence[], bidirectional status
```
