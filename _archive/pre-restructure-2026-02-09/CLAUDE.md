...
## HOSTING DIRECTIVE (CRITICAL)

**⚠️ The Continuum Report is LOCALLY HOSTED on WoodsDen — Tower is GONE.**

| Directive | Details |
|-----------|---------|
| **Host** | WoodsDen (local Windows PC) |
| **Project Root** | `C:\Users\Xx LilMan xX\Documents\Claude Docs\Continuum\` |
| **Tower/T:\** | **PERMANENTLY GONE** — do not reference |
| **Paperless** | **GONE** — need local alternative for OCR |

All development, serving, and infrastructure runs locally on WoodsDen. There is no network share.
...
| Local Dev Server | http://localhost:8081 (when running) |
| GitHub Repo | WoodsBandit/the-continuum-report |
...
### Host: WoodsDen (Local PC)

**All Continuum infrastructure runs locally on WoodsDen. Tower is gone.**

| Component | Status | Notes |
|-----------|--------|-------|
| Website serving | **LOCAL** | `python -m http.server 8081 --directory website` |
| Development | **LOCAL** | All code changes happen here |
| Git operations | **LOCAL** | Push to GitHub from here |
| Cloudflare tunnel | **LOCAL** | Connect tunnel to localhost:8081 |
| OCR/Document processing | **TODO** | Need local solution (Tesseract, etc.) |
...
- **Routing:** Cloudflare tunnel → localhost:8081 (WoodsDen)
...
1. **Search** for completed item across all .md files:
   ```bash
   grep -rln "TASK_KEYWORDS" "C:\Users\Xx LilMan xX\Documents\Claude Docs\Continuum\*.md"
   ```
2. **Update ALL matching files** - mark `[x]`, remove from pinned lists, add dates
3. **Update MASTER_TODO_LIST.md** (local is now canonical):
...
|------|----------|----------|
| MASTER_TODO_LIST.md | Local project root | **Canonical** |
| CLAUDE.md | Local project root | Update if contains item |
